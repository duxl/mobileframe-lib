...
public class XListView extends ListView implements OnScrollListener {
	...
	private final static float OFFSET_RADIO = 1.8f; // support iOS like pull
													// feature.
	private LoadMore mLoadMoreType = LoadMore.AUTOMATIC;

	/**
	 * 设置加载更多的方式，需setLoadMoreEnable(true)才有效，默认自动加载
	 * 
	 * @param mLoadMore
	 */
	public void setLoadMoreType(LoadMore loadMore) {
		this.mLoadMoreType = loadMore;
	}
	...
	private void initWithContext(Context context) {
		...
		setLoadMoreEnable(mLoadMoreEnable);
	...
	public void setLoadMoreEnable(boolean enable) {
		...
		if (!mLoadMoreEnable) {
			...
			mFooterView.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
					if (mLoadMoreType == LoadMore.MANUAL) {
						startLoadMore();
						mFooterView.setOnClickListener(null);
					}
				}
	...
	public void showRefresh() {
		System.out.println("mHeaderView.getHeight()=" + (mHeaderViewHeight + 100));
		mPullRefreshing = true;
		mHeaderView.setState(XListViewHeader.STATE_REFRESHING);
		setSelection(0);
		updateHeaderHeight(mHeaderViewHeight + 100);
		resetHeaderHeight();
	}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		default:
			...
			if (getFirstVisiblePosition() == 0) {
				...
				// invoke load more.
				if (mLoadMoreEnable && mFooterView.getBottomMargin() > PULL_LOAD_MORE_DELTA && !mPullLoading) {
					startLoadMore();
				}
				resetFooterHeight();
			}
